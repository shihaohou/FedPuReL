...
class ModifiedResNet_CAPFL(nn.Module):
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        super().__init__()
        self.output_dim = output_dim
        self.input_resolution = input_resolution
...
class Transformer_fedtpg(nn.Module):
...
class VisionTransformer_CAPFL(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "CAPFL":
                self.visual = ModifiedResNet_CAPFL(
            ...
            if trainer == "GLP_OT":
                ...
                self.visual = VisionTransformer_CAPFL(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
        ...
        if trainer == "FedTPG":
            self.transformer = Transformer_fedtpg(
