...
def load_aircraft_LT_data(datadir, imb_factor, imb_type):
    """
    Load FGVC-Aircraft long-tail data by calling the function from aircraft_LT.py
    """
    from datasets.aircraft_LT import load_aircraft_LT_data as _load_aircraft_LT_data
    return _load_aircraft_LT_data(datadir, imb_factor, imb_type)


def load_dtd_LT_data(datadir, imb_factor, imb_type):
    import os
    import random
    import numpy as np
    ...
    assert os.path.isdir(image_dir), f"DTD images dir not found: {image_dir}"
    ...
    per_class_items = [[] for _ in range(num_classes)]
    ...
    _, list_label2indices_train_new = train_long_tail(list_label2indices, num_classes, imb_factor, imb_type)
    lt_train_indices = flatten_list(list_label2indices_train_new)
...
def load_oxford_pets_LT_data(datadir, imb_factor, imb_type):
    """
    Delegate Oxford Pets LT loading to datasets.oxford_pets_LT implementation,
    which parses annotations and class names correctly (37 breeds).
    """
    from datasets.oxford_pets_LT import load_oxford_pets_LT_data as _load_oxford_pets_LT_data
    return _load_oxford_pets_LT_data(datadir, imb_factor, imb_type)

def load_stanford_dogs_LT_data(datadir, imb_factor, imb_type):
    from datasets.stanford_dogs_LT import load_stanford_dogs_LT_data as _load_stanford_dogs_LT_data
    return _load_stanford_dogs_LT_data(datadir, imb_factor, imb_type)


def load_stanford_cars_LT_data(datadir, imb_factor, imb_type):
    from datasets.stanford_cars_LT import load_stanford_cars_LT_data as _load_stanford_cars_LT_data
    return _load_stanford_cars_LT_data(datadir, imb_factor, imb_type)


def load_food101_LT_data(datadir, imb_factor, imb_type):
    from datasets.food101_LT import load_food101_LT_data as _load_food101_LT_data
    return _load_food101_LT_data(datadir, imb_factor, imb_type)
