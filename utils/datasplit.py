...
from utils.dataloader import load_mnist_data, load_fmnist_data, load_fmnist_LT_data, load_cifar10_data, load_cifar100_data, load_cifar10_LT_data, load_cifar100_LT_data, load_svhn_data, load_celeba_data, load_femnist_data, load_dtd_LT_data, load_aircraft_LT_data, load_oxford_pets_LT_data, load_stanford_dogs_LT_data, load_stanford_cars_LT_data, load_food101_LT_data
...
def partition_data(dataset, datadir, partition, n_parties, beta=0.4, logdir=None, seed=None):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
...
def partition_data_LT(dataset, datadir, partition, n_parties, imb_factor, imb_type, beta, logdir=None, seed=1):
    ...
    if dataset == 'cifar10_LT':
        ...
        X_train, y_train, X_test, y_test, data_train, data_test, lab2cname, classnames = load_dtd_LT_data(datadir, imb_factor, imb_type)
        y = np.concatenate([y_train, y_test], axis=0)
    elif dataset == 'aircraft_LT':
        X_train, y_train, X_test, y_test, data_train, data_test, lab2cname, classnames = load_aircraft_LT_data(datadir, imb_factor, imb_type)
        y = np.concatenate([y_train, y_test], axis=0)
    elif dataset == 'oxford_pets_LT':
        X_train, y_train, X_test, y_test, data_train, data_test, lab2cname, classnames = load_oxford_pets_LT_data(datadir, imb_factor, imb_type)
        y = np.concatenate([y_train, y_test], axis=0)
    elif dataset == 'stanford_dogs_LT':
        X_train, y_train, X_test, y_test, data_train, data_test, lab2cname, classnames = load_stanford_dogs_LT_data(datadir, imb_factor, imb_type)
        y = np.concatenate([y_train, y_test], axis=0)
    elif dataset == 'stanford_cars_LT':
        X_train, y_train, X_test, y_test, data_train, data_test, lab2cname, classnames = load_stanford_cars_LT_data(datadir, imb_factor, imb_type)
        y = np.concatenate([y_train, y_test], axis=0)
    elif dataset == 'food101_LT':
        X_train, y_train, X_test, y_test, data_train, data_test, lab2cname, classnames = load_food101_LT_data(datadir, imb_factor, imb_type)
        y = np.concatenate([y_train, y_test], axis=0)
...
def _build_iid_test_map_by_label(y_test, n_parties, seed=None):
    ...
    for label in unique_labels:
        idx_k = np.where(y_test == label)[0]
        np.random.shuffle(idx_k)
        splits = np.array_split(idx_k, n_parties)
        for client_id in range(n_parties):
            net_dataidx_map_test[client_id] = np.append(net_dataidx_map_test[client_id], splits[client_id])

    # 保证为 int64
    for client_id in range(n_parties):
        net_dataidx_map_test[client_id] = net_dataidx_map_test[client_id].astype(np.int64)

    return net_dataidx_map_test


def partition_data_LT_test_iid(dataset, datadir, partition, n_parties, imb_factor, imb_type, beta, logdir=None, seed=1):


    data_train, data_test, lab2cname, classnames, net_dataidx_map_train, _, traindata_cls_counts, _, y_train = \
        partition_data_LT(dataset, datadir, partition, n_parties, imb_factor, imb_type, beta, logdir, seed)

    if dataset == 'cifar10_LT':
        _, _, _, y_test, _, _, _, _ = load_cifar10_LT_data(datadir, imb_factor, imb_type)
    elif dataset == 'cifar100_LT':
        _, _, _, y_test, _, _, _, _ = load_cifar100_LT_data(datadir, imb_factor, imb_type)
    elif dataset == 'fmnist_LT':
        _, _, _, y_test, _, _, _, _ = load_fmnist_LT_data(datadir, imb_factor, imb_type)
    elif dataset == 'dtd_LT':
        _, _, _, y_test, _, _, _, _ = load_dtd_LT_data(datadir, imb_factor, imb_type)
    elif dataset == 'aircraft_LT':
        _, _, _, y_test, _, _, _, _ = load_aircraft_LT_data(datadir, imb_factor, imb_type)
    else:
        raise ValueError(f"Unsupported dataset for LT IID test split: {dataset}")

    net_dataidx_map_test = _build_iid_test_map_by_label(y_test, n_parties, seed)
    ...
    return (
        ...
        traindata_cls_counts, testdata_cls_counts, y_train
    )


def partition_data_test_iid(dataset, datadir, partition, n_parties, beta=0.4, logdir=None, seed=None):


    data_train, data_test, lab2cname, classnames, net_dataidx_map_train, _, traindata_cls_counts, _, y_train = \
        partition_data(dataset, datadir, partition, n_parties, beta, logdir, seed)


    if dataset == 'mnist':
        _, _, _, y_test = load_mnist_data(datadir)
    elif dataset == 'fmnist':
        _, _, _, y_test, _, _, _, _ = load_fmnist_data(datadir)
    elif dataset == 'cifar10':
        _, _, _, y_test, _, _, _, _ = load_cifar10_data(datadir)
    elif dataset == 'cifar100':
        _, _, _, y_test, _, _, _, _ = load_cifar100_data(datadir)
    elif dataset == 'svhn':
        _, _, _, y_test = load_svhn_data(datadir)
    elif dataset == 'celeba':
        _, _, _, y_test = load_celeba_data(datadir)
    elif dataset == 'femnist':
        _, _, _, _, y_test, _ = load_femnist_data(datadir)
    elif dataset == 'generated':

        y_test = np.load("data/generated/y_test.npy")
    else:
        raise ValueError(f"Unsupported dataset for IID test split: {dataset}")
