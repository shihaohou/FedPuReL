...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'GradPur',
                      "vision_depth": 0,
...
class GradPur(TrainerX):
    ...
    def build_model(self):
        ...
        # build criterion
        # if cfg.LOSS.NAME == "GradPur":
        self.criterion = GradPurLoss(T=self.cfg.LOSS.T)
    ...
    def forward_backward(self, batch):
        ...
        with torch.no_grad():
            ...
            ent_t = entropy_from_logits(zs_clip_output)  # teacher entropy
            ent_s = entropy_from_logits(output)          # student entropy
            h_star = 0.5 * (ent_t + ent_s)

            tau_t = search_temperature_per_sample(zs_clip_output, h_star)
            tau_s = search_temperature_per_sample(output, h_star)

            p_t = F.softmax(zs_clip_output / tau_t.view(-1, 1), dim=-1)

        p_s = F.softmax(output / tau_s.view(-1, 1), dim=-1)

        kl = (p_t * (torch.log(p_t.clamp_min(1e-12)) - torch.log(p_s.clamp_min(1e-12)))).sum(dim=-1)
        kl_loss = kl.mean()


        xe_loss = F.cross_entropy(output, label)

        self.GradPur_backward_and_update(xe_loss, kl_loss, self.cfg.LOSS.LAMBDA)
