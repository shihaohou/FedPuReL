...
class CLIP(nn.Module):
    """Zero-shot CLIP model for computing KL divergence"""
    def __init__(self, cfg, classnames):
        super().__init__()
        print(f"Loading CLIP for KL computation (backbone: {cfg.MODEL.BACKBONE.NAME})")
        clip_model = load_clip_to_cpu(cfg)
        clip_model.float()

        # Use the same template as CustomCLIP
...
class FedClip(TrainerX):
    ...
    def build_model(self):
        ...
        self.gradient_collector = {
            'batch_gradients': [],  # 存储每个batch的梯度
            'batch_count': 0,       # batch计数
            'grad_sum': None,       # 梯度累加和
        }
        self.local_avg_gradient = None  # 本地平均梯度
    ...
    def get_local_gradient(self):
        return self.local_avg_gradient

    def after_train(self, idx=-1, epoch=0, is_fed=False):

        if is_fed and idx >= 0:
            local_avg_grad = self.compute_local_avg_gradient()
            if local_avg_grad is not None:
                print(f"FedCLIP Client {idx}: Computed local average gradient with shape {local_avg_grad.shape}")
        
        super().after_train(idx, epoch, is_fed)
